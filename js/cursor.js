...
class WholeCursor {
  constructor() {
    this.bindMethods();
    this.canvas = document.querySelector('#cursor_canvas');
  ...
  bindMethods() {
    ['draw', 'onRS', 'getPos', 'onPress', 'onRelease'].forEach(method => this[method] = this[method].bind(this));
  }

  setcanvas() {
    this.canvas.style.height = `${this.bs.h}px`;
    this.canvas.style.width = `${this.bs.w}px`;
    this.canvas.height = this.bs.h;
    this.canvas.width = this.bs.w;
  ...
  draw() {
    ...
    this.ctx.strokeStyle = 'rgba(200, 200, 255, 0.7)';
    this.ctx.stroke();
    this.ctx.closePath();
    this.ctx.beginPath();
    this.ctx.arc(last.x, last.y, radius - 4, start * Math.PI, end * Math.PI);
    this.ctx.lineWidth = 2;
    this.ctx.strokeStyle = '#a460ed';
    this.ctx.stroke();
    this.ctx.closePath();
  ...
  addListeners() {
    window.addEventListener('resize', this.onRS);
...
const wholeCursor = new WholeCursor();